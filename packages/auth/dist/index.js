...
function buildDevBypassSessionPayload(appId) {
...
export async function decodePortalSession(options = {}) {
    const { cookieHeader, cookieNames, appId, secret, debug = truthyValues.has(String(process.env.NEXTAUTH_DEBUG ?? '').toLowerCase()), request, } = options;
...
function resolveConfiguredOrigin(candidates, errorMessage) {
    for (const candidate of candidates) {
...
export function resolveAppAuthOrigin(options) {
    void options;
    return resolveConfiguredOrigin([
...
export async function hasPortalSession(options) {
    const { request, appId, cookieNames, debug = options.debug ?? truthyValues.has(String(process.env.NEXTAUTH_DEBUG ?? '').toLowerCase()), } = options;
    const names = Array.from(new Set((cookieNames && cookieNames.length > 0)
        ? cookieNames
        : getCandidateSessionCookieNames(appId)));
...
function isLocalhostOrigin(raw) {
...
function hasLocalhostBypassContext(options) {
    const envCandidates = [
        process.env.NEXTAUTH_URL,
        process.env.PORTAL_AUTH_URL,
        process.env.NEXT_PUBLIC_PORTAL_AUTH_URL,
...
function isDevAuthBypassEnabled(options) {
...
function buildDevBypassAuthz(appId) {
    const apps = {};
    if (appId) {
        apps[appId] = {
            departments: [],
...
export async function getCurrentAuthz(request, options) {
    ...
    if (!decoded) {
        if (isDevAuthBypassEnabled({ request: request })) {
            if (debug) {
