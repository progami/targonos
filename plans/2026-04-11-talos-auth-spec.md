...
## Files Reviewed
- Required context: `app-manifest.json`, `dev.local.apps.json`, `plans/2026-04-11-cross-app-ci-smoke-spec.md`, `plans/2026-04-11-talos-test-plan.md`, `apps/talos/src/middleware.ts`, `apps/talos/next.config.js`, `apps/talos/src/app/page.tsx`.
...
- The existing smoke evidence in `plans/2026-04-11-cross-app-ci-smoke-spec.md` already recorded `POST /talos/api/tenant/select -> 401`, `GET /talos/api/portal/session -> 401`, and `GET /talos/api/tenant/current -> 401` during the Talos landing flow.
...
## Confirmed Issues
- Public tenant bootstrap is not auth-safe. `apps/talos/src/components/tenant/WorldMap.tsx` explicitly treats failed `/api/tenant/current` loads as “show all regions and let select fail later,” and the smoke spec in `plans/2026-04-11-cross-app-ci-smoke-spec.md` confirms the resulting raw `401` failures on `/api/tenant/current`, `/api/tenant/select`, and `/api/portal/session`.
...
- Browser smoke should assert no `401/403` during tenant bootstrap, no React hydration/runtime error, and no wrong-origin redirect in the Talos launch path described in `plans/2026-04-11-cross-app-ci-smoke-spec.md`.

## Cross-App Notes
- `apps/sso/lib/apps.ts` resolves Talos from `dev.local.apps.json`, which maps Talos to `3201`, but `apps/talos/package.json` still starts Talos on `3001`; the smoke spec in `plans/2026-04-11-cross-app-ci-smoke-spec.md` already observed the repo running on `3001`/`3000` instead of the `3201`/`3200` expected by `apps/talos/.env.local`.
