...
## Goal
Document the `sso` navigation behavior that is actually implemented today, with emphasis on launcher href resolution, local-vs-hosted app topology, callback/relay transitions, login/logout routing, and dead or looping routes. This spec is discovery only and is grounded in code plus `plans/2026-04-11-cross-app-ci-smoke-spec.md` and `plans/2026-04-11-sso-test-plan.md`.

## Files Reviewed
- Inputs and existing specs: `app-manifest.json`, `dev.local.apps.json`, `plans/2026-04-11-cross-app-ci-smoke-spec.md`, `plans/2026-04-11-sso-test-plan.md`, `plans/2026-04-11-sso-auth-spec.md`, `plans/2026-04-11-plutus-auth-spec.md`.
...
- `plans/2026-04-11-cross-app-ci-smoke-spec.md` already observed runtime symptoms consistent with this code: localhost Google auth drift, per-app port divergence, and app entry paths that do not line up with the portal’s launch map.
...
- Which runtime env source produced the hosted Google `redirect_uri` seen in `plans/2026-04-11-cross-app-ci-smoke-spec.md` despite the checked-in localhost values in `apps/sso/.env.local`? No direct evidence yet.
