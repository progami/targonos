...
## Files Reviewed
- Repo/app topology and existing specs: `app-manifest.json`, `dev.local.apps.json`, `plans/2026-04-11-cross-app-ci-smoke-spec (COMPLETED).md`, `plans/2026-04-11-xplan-test-plan.md`.
...
## Confirmed Issues
- xPlan can render the workbook shell without a usable xPlan session. `apps/xplan/app/[sheet]/page.tsx:2230-2277,2555` calls `auth()` but does not block on `null`; it builds an actor from the nullable session, resolves strategy access, and still renders `SetupWorkspace`. That matches the smoke finding in `plans/2026-04-11-cross-app-ci-smoke-spec (COMPLETED).md` where Setup rendered while `/api/v1/xplan/assignees` returned `401`.
