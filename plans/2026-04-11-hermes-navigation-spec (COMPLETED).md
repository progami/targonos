...
- Local standalone origin/topology is inconsistent with the launcher map. `plans/2026-04-11-cross-app-ci-smoke-spec (COMPLETED).md` recorded Hermes at `http://localhost:3014/hermes`, while `dev.local.apps.json` expects `http://localhost:3214/hermes`. Since middleware builds login callbacks from env-derived app origin in `apps/hermes/src/middleware.ts`, this drift directly affects deep-link return behavior.
