...
## Likely Root Causes
- The public website is routing users into statically split Caelum Star segments that are not being proved in browser smoke. The chunk failures in `plans/2026-04-11-cross-app-ci-smoke-spec (COMPLETED).md` show that promoted routes can ship in a broken state even when build checks pass.
...
- `plans/2026-04-11-cross-app-ci-smoke-spec (COMPLETED).md` already identifies `website` as the clearest hard runtime failure in the current suite because the app loads initially but breaks as soon as promoted route chunks are fetched.
