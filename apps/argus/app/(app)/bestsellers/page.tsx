...
export default async function BestsellersPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader title="Bestsellers" subtitle="Monitor bestseller lists and category rankings.">
        <Button asChild size="sm">
          <Link href="/targets/new"><Plus className="mr-1.5 h-4 w-4" />Add Category</Link>
        </Button>
      </PageHeader>
      ...
      {targets.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground">No bestseller targets yet.</p>
            <Button asChild size="sm" className="mt-4">
              <Link href="/targets/new">Add your first category</Link>
            </Button>
          </CardContent>
        </Card>
      ) : (
                  ...
                  return (
                    <tr key={t.id} className="border-b transition-colors last:border-0 hover:bg-muted/50">
                      <td className="px-4 py-3">
                        <Link href={`/bestsellers/${t.id}`} className="font-medium text-foreground hover:text-primary">
                          {t.label}
                        </Link>
                        {t.sourceUrl && (
                          <p className="mt-0.5 truncate text-2xs text-muted-foreground">{t.sourceUrl}</p>
                        )}
                      </td>
                      <td className="px-4 py-3">
