...
test('weekly browser collectors require market-specific source config', () => {
  ...
  assert.match(scaleInsights, /require_market_env WPR_HERO_ASIN/)
...
test('ScaleInsights reruns reuse an existing same-week XLSX', () => {
  ...
  assert.match(scaleInsights, /target_matches_requested_export/)
  assert.match(scaleInsights, /Saved: \$\{PREFIX\}_SI-KeywordRanking\.xlsx already current/)
})

test('ScaleInsights export URL is scoped to market country and ASIN', () => {
  assert.match(scaleInsights, /ASIN="\$\(require_market_env WPR_HERO_ASIN\)"/)
  assert.match(scaleInsights, /download_url="\$\{TARGET_URL\}\?asin=\$\{ASIN\}&countrycode=\$\{COUNTRY_CODE\}&from=\$\{START_DATE\}&to=\$\{END_DATE\}&handler=Excel"/)
  assert.match(scaleInsights, /payload\.countryCode !== countryCode/)
  assert.match(scaleInsights, /payload\.asin !== asin/)
