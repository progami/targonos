...
async function main(): Promise<void> {
  ...
  // Delete settlement JEs first. If any settlement JE is matched to a bank-feed transaction, QBO blocks deletion.
  // We fail fast before deleting processing JEs so the system doesn't end up in a partially-reset state.
  targets.sort((a, b) => {
  ...
  const settlementTargets = targets.filter((t) => t.kind === 'settlement');
  const otherTargets = targets.filter((t) => t.kind !== 'settlement');

  for (const target of settlementTargets) {
    await deleteTarget(target);
  }

  const failedSettlementDeletions = deletions.filter((d) => !d.ok).map((d) => d.journalEntryId);
      ...
      JSON.stringify(
        {
          ...
          qboLinks: deletions
            .filter((d) => !d.ok)
            .map((d) => ({ journalEntryId: d.journalEntryId, qboUrl: buildQboJournalHref(d.journalEntryId) })),
        },
        null,
        2,
      ),
