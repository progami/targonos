...
type DbTableIdentifier = typeof import('@/lib/db').dbTableIdentifier;
...
function parseArgs(argv: string[]): CliOptions {
  ...
  if (post && humanApproval !== HUMAN_APPROVAL_PHRASE) {
    throw new Error(`Settlement rematch posting requires --human-approval "${HUMAN_APPROVAL_PHRASE}"`);
  }

  return { post, startDate, targetSettlementIds, humanApproval };
}

async function fetchAuditInvoiceSummaries(
  db: DbClient,
  dbTableIdentifier: DbTableIdentifier,
): Promise<AuditInvoiceSummary[]> {
...
async function main(): Promise<void> {
  ...
  const settlementProcessingModule = await import('@/lib/plutus/settlement-processing');
  const db: DbClient = dbModule.db;
  const dbTableIdentifier: DbTableIdentifier = dbModule.dbTableIdentifier;
  const computeSettlementPreview: ComputeSettlementPreview = settlementProcessingModule.computeSettlementPreview;
  const processSettlement: ProcessSettlement = settlementProcessingModule.processSettlement;
  ...
  const invoices = await fetchAuditInvoiceSummaries(db, dbTableIdentifier);
