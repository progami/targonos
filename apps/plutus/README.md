...
Plutus is the finance workspace for Amazon settlements plus QuickBooks Online (QBO).
...
- Sync Amazon settlements via SP-API Finances and process settlements (posts COGS JE and P&L reclass JE)
- Bills mapping to build SKU cost basis for inventory COGS
- Transactions and benchmarking analytics

Persistence:
- Uses Postgres via Prisma (schema `plutus`) for setup, transaction data, processing history, and bill mappings.
