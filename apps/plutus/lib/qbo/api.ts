 ...
 * Fetches the full bill, applies account changes to specified lines, and POSTs the update.
 */
export async function updateBillLineAccounts(
  ...
  // First fetch the full bill to get its complete Line array
  ...
  // Apply account changes to the specified lines
  ...
  const response = await fetchWithTimeout(updateUrl, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${accessToken}`,
      Accept: 'application/json',
