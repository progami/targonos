...
test('TargonOS home renders the authenticated launcher', async ({ page }) => {
  ...
  await expect(page.getByText('E-commerce Catalouge Monitoring')).toBeVisible()
  ...
  await expect(page.getByText('Settlment Controller')).toBeVisible()
  ...
  await expect(page.getByText('Open public surface')).toHaveCount(1)
