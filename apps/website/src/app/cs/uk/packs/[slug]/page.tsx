...
export function generateStaticParams() {
  return productsUK.map((p) => ({ slug: p.slug }));
}

export function generateMetadata({ params }: PageProps): Metadata {
  const p = productsUK.find((x) => x.slug === params.slug);
...
export default function ProductDetailPage({ params }: PageProps) {
  ...
  const primary = productsUK.find((x) => x.primary)!;
                ...
                <div className="md:col-span-5 md:flex md:justify-end">
                  <div className="flex flex-wrap gap-3">
                    <Button asChild variant="outline" className="border-white/15 bg-white/[0.07] text-white hover:bg-white/[0.12]">
                      <Link href="/cs/uk/packs">See all packs</Link>
                    </Button>
                    <Button asChild variant="accent">
