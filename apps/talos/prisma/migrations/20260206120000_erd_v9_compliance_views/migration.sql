-- ERD v9 compatibility views for Talos purchase-order flow.
-- This keeps the current operational schema intact while exposing an ERD-aligned shape.
...
SELECT
  po."id" AS "po_id",
  ci."ci_id",
  COALESCE(sq."qty_on_shipment", 0) AS "qty_on_shipment"
FROM "purchase_orders" po
JOIN "commercial_invoice" ci
  ON ci."ci_id" = po."id"
LEFT JOIN shipment_qty sq
