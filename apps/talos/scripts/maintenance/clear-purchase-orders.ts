...
async function clearTenant(tenant: TenantCode, dryRun: boolean) {
  ...
  try {
    ...
    const txCount = await prisma.inventoryTransaction.count({
      where: {
        OR: [{ purchaseOrderId: { not: null } }, { purchaseOrderLineId: { not: null } }],
      },
    })

    if (dryRun) {
      console.log(`[${tenant}] Purchase orders=${poCount} PO transactions=${txCount}`)
      return
    }
    ...
    console.log(
      `[${tenant}] Deleted ${poResult.count} purchase orders and ${txResult.count} inventory transactions`
    )
  } finally {
