...
function showHelp() {
  console.log(`
Add Warehouse Billing Config

Adds the warehouses.billing_config column used by warehouse billing rules.

Usage:
  pnpm --filter @targon/talos tsx scripts/migrations/add-warehouse-billing-config.ts [options]
...
async function applyForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  console.log(`\n[${tenant}] Ensuring warehouses.billing_config exists`)
