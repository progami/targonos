...
/**
 * Reshuffle Inbound document stages — moves documents one stage earlier to match
 ...
 *
 * Usage:
 *   pnpm --filter @targon/talos tsx scripts/migrations/ensure-inbound-document-stage-reshuffle.ts [options]
 *
 * Options:
...
function showHelp() {
  console.log(`
Ensure Inbound Document Stage Reshuffle

Moves existing Inbound documents one stage earlier to match the updated document
...
function buildUpdateSql(move: StageMove): string {
  return `
    ...
    WHERE d."stage" = '${move.fromStage}'
      ...
      AND NOT EXISTS (
        SELECT 1 FROM "inbound_order_documents" e
        WHERE e."inbound_order_id" = d."inbound_order_id"
...
async function applyForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  console.log(`\n[${tenant}] Reshuffling Inbound document stages`)
...
async function main() {
  ...
  console.log('=== Inbound Document Stage Reshuffle ===')
