...
import { PURCHASE_ORDER_BASE_CURRENCY } from '../../src/lib/constants/cost-currency'
...
function showHelp() {
  console.info(`
Normalize Purchase Order Base Currency

Backfills purchase-order product and manual PO-cost records to ${PURCHASE_ORDER_BASE_CURRENCY}.
Freight / forwarding rows are left untouched so they can remain USD or GBP.

Usage:
  pnpm --filter @targon/talos db:migrate:po-base-currency [options]
...
async function applyForSchema(
  ...
  const counts = await client.query<CountRow>(`
    SELECT
      (
        ...
        WHERE currency IS DISTINCT FROM '${PURCHASE_ORDER_BASE_CURRENCY}'
      ) AS line_count,
      (
        SELECT COUNT(*)::text
        ...
        WHERE purchase_order_id IS NOT NULL
          ...
          AND currency IS DISTINCT FROM '${PURCHASE_ORDER_BASE_CURRENCY}'
      ...
      (
        ...
        WHERE duty_amount IS NOT NULL
          AND duty_amount <> 0
          AND duty_currency IS DISTINCT FROM '${PURCHASE_ORDER_BASE_CURRENCY}'
      ) AS duty_count,
      (
        SELECT COUNT(*)::text
        FROM purchase_order_forwarding_costs
  ...
  console.info(`${banner} purchase_order_lines to normalize=${lineCount}`)
  console.info(`${banner} manual PO financial rows to normalize=${manualFinancialCount}`)
