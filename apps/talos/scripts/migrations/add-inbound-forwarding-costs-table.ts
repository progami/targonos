...
function showHelp() {
  console.log(`
Add Inbound Forwarding Costs Table

Creates inbound_order_forwarding_costs in each tenant schema.
Used to capture Inbound-level forwarding costs (CostCategory.Forwarding) and allocate
them into CostLedger at receipt time.

Usage:
  pnpm --filter @targon/talos tsx scripts/migrations/add-inbound-forwarding-costs-table.ts [options]
...
async function applyForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  console.log(`\n[${tenant}] Ensuring inbound_order_forwarding_costs exists`)
  ...
  const ddlStatements = [
    ...
    `,
    `ALTER TABLE "inbound_order_forwarding_costs" DROP CONSTRAINT IF EXISTS "inbound_order_forwarding_costs_inbound_order_id_fkey"`,
    `
    ...
    `,
    `ALTER TABLE "inbound_order_forwarding_costs" DROP CONSTRAINT IF EXISTS "inbound_order_forwarding_costs_warehouse_id_fkey"`,
    `
      ALTER TABLE "inbound_order_forwarding_costs"
    ...
    `,
    `ALTER TABLE "inbound_order_forwarding_costs" DROP CONSTRAINT IF EXISTS "inbound_order_forwarding_costs_cost_rate_id_fkey"`,
    `
      ALTER TABLE "inbound_order_forwarding_costs"
