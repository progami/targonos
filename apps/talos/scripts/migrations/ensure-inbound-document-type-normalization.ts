...
/**
 * Normalize legacy Inbound document types to match the current UI expectations.
 *
 ...
 *
 * Usage:
 *   pnpm --filter @targon/talos exec tsx scripts/migrations/ensure-inbound-document-type-normalization.ts [options]
 *
 * Options:
...
import { InboundOrderDocumentStage, InboundOrderLineStatus } from '@targon/prisma-talos'
...
function showHelp() {
  console.log('=== Inbound Document Type Normalization ===\n')
  console.log('Options:')
...
async function renameTransactionCertificate(params: { prisma: PrismaClient; dryRun: boolean }) {
  const count = await params.prisma.inboundOrderDocument.count({
    where: {
  ...
  for (const row of rows) {
    const already = await params.prisma.inboundOrderDocument.findFirst({
      where: {
    ...
    await params.prisma.inboundOrderDocument.update({
      where: { id: row.id },
...
async function expandPlainBoxArtwork(params: { prisma: PrismaClient; dryRun: boolean; limit: number }) {
  const baseDocs = await params.prisma.inboundOrderDocument.findMany({
    where: {
  ...
  if (baseDocs.length === 0) {
    return { created: 0, sample: [] as Array<{ inboundOrderId: string; documentType: string }> }
  }

  let created = 0
  const sample: Array<{ inboundOrderId: string; documentType: string }> = []

  for (const doc of baseDocs) {
    const lines = await params.prisma.inboundOrderLine.findMany({
      where: {
        inboundOrderId: doc.inboundOrderId,
    ...
    if (params.dryRun) {
      ...
      for (const row of rows) {
        ...
        sample.push({ inboundOrderId: row.inboundOrderId, documentType: row.documentType })
      }
      continue
    }

    const result = await params.prisma.inboundOrderDocument.createMany({
      data: rows,
      skipDuplicates: true,
    })
...
async function applyForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  console.log(`\n[${tenant}] Normalizing Inbound document types`)
  ...
  if (artworkResult.sample.length > 0) {
    ...
    for (const entry of artworkResult.sample) {
      console.log(`[${tenant}] ${entry.inboundOrderId} -> ${entry.documentType}`)
    }
  }
