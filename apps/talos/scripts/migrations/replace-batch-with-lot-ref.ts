...
Also:
- Ensures inbound_order_lines.production_date exists
- Fixes the inbound_order_lines unique constraint to (inbound_order_id, sku_code)
- Backfills inventory/ledger lot_ref values from inbound_order_lines when linked
...
function dropSkuBatchEnforcement(): string[] {
  return [
    `
      -- ERD v9 removed the SKU batch concept; lots are generated per Inbound line.
...
function dropInboundOrderLineBatchLot(): string {
  return `
    -- Ensure legacy inbound_order_lines.batch_lot does not survive the lot_ref migration.
    DO $$
    BEGIN
      IF EXISTS (
        SELECT 1 FROM information_schema.columns
        ...
        ALTER TABLE "inbound_order_lines" DROP COLUMN "batch_lot";
      END IF;
    END $$;
  `
}
...
async function applyForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  const statements = [
    ...
    renameColumnIfNeeded('outbound_order_lines', 'batch_lot', 'lot_ref'),
    renameColumnIfNeeded('goods_receipt_lines', 'batch_lot', 'lot_ref'),

    // ERD v9 lot PK = (inbound_id, sku_id) so enforce one SKU per Inbound line.
    `ALTER TABLE "inbound_order_lines" DROP CONSTRAINT IF EXISTS "inbound_order_lines_inbound_order_id_sku_code_batch_lot_key"`,
    `DROP INDEX IF EXISTS "inbound_order_lines_inbound_order_id_sku_code_batch_lot_key"`,
    `
      WITH dups AS (
        SELECT
          "inbound_order_id",
        ...
        GROUP BY "inbound_order_id", "sku_code"
        HAVING COUNT(*) > 1
      ),
      id_map AS (
      ...
      update_inventory_transactions AS (
        ...
        SET "inbound_order_line_id" = m.keep_id
        FROM id_map m
        WHERE it."inbound_order_line_id" = m.old_id
      ...
      update_financial_ledger AS (
        ...
        SET "inbound_order_line_id" = m.keep_id
        FROM id_map m
        WHERE fl."inbound_order_line_id" = m.old_id
          AND m.old_id <> m.keep_id
        RETURNING fl."id"
      ),
      update_goods_receipt_lines AS (
        ...
        SET "inbound_order_line_id" = m.keep_id
        FROM id_map m
        WHERE grl."inbound_order_line_id" = m.old_id
          AND m.old_id <> m.keep_id
        RETURNING grl."id"
      ),
      update_warehouse_invoice_lines AS (
        ...
        SET "inbound_order_line_id" = m.keep_id
        FROM id_map m
        WHERE wil."inbound_order_line_id" = m.old_id
          AND m.old_id <> m.keep_id
        RETURNING wil."id"
      ),
      delete_dups AS (
        DELETE FROM "inbound_order_lines" pol
        USING id_map m
    ...
    `,
    `CREATE UNIQUE INDEX IF NOT EXISTS "inbound_order_lines_inbound_order_id_sku_code_key"
      ON "inbound_order_lines"("inbound_order_id", "sku_code")`,
    ...
    `
      ...
      WHERE it."inbound_order_line_id" IS NOT NULL
        AND it."inbound_order_line_id" = pol."id"
    ...
    `
      ...
      SET "lot_ref" = pol."lot_ref"
      FROM "inbound_order_lines" pol
      WHERE it."inbound_order_line_id" IS NULL
        AND it."inbound_order_id" IS NOT NULL
        AND it."inbound_order_id" = pol."inbound_order_id"
    ...
    `
      ...
      SET "lot_ref" = pol."lot_ref"
      FROM "inbound_order_lines" pol
      WHERE fl."inbound_order_line_id" IS NOT NULL
        AND fl."inbound_order_line_id" = pol."id"
    ...
    `
      ...
      SET "lot_ref" = pol."lot_ref"
      FROM "inbound_order_lines" pol
      WHERE fl."inbound_order_line_id" IS NULL
        AND fl."inbound_order_id" IS NOT NULL
        AND fl."inbound_order_id" = pol."inbound_order_id"
    ...
    `
      ...
      SET "lot_ref" = pol."lot_ref"
      FROM "inbound_order_lines" pol
      WHERE grl."inbound_order_line_id" IS NOT NULL
        AND grl."inbound_order_line_id" = pol."id"
        AND grl."lot_ref" IS DISTINCT FROM pol."lot_ref"
    `,

    // Remove legacy SKU batch enforcement + drop the last remaining batch_lot column on Inbound lines.
    ...dropSkuBatchEnforcement(),
    dropInboundOrderLineBatchLot(),
  ]
