...
const UNITS_FIRST_MIGRATION = '20260108190000_inbound_line_units_first'
...
function showHelp() {
  console.log(`
Fix legacy Inbound line quantity semantics

Early Talos Inbound lines stored "quantity" as UNITS. Later migrations introduced:
  - units_ordered (units)
  - units_per_carton
  - total_cost
  - quantity (cartons)
...
This script repairs legacy rows by:
  - treating existing inbound_order_lines.quantity as units
...
Usage:
  pnpm --filter @targon/talos tsx scripts/migrations/fix-inbound-line-legacy-units.ts [options]
...
async function applyForSchema(
  ...
  const candidateLines = await client.query<CandidateLine>(
    `
      ...
      JOIN inbound_orders po ON inbound.id = pol.inbound_order_id
      ...
      ORDER BY inbound.inbound_number, pol.sku_code, pol.batch_lot
    `,
    [cutoff]
  )
  ...
  if (options.dryRun) {
    for (const row of candidateLines.rows) {
      ...
      console.log(
        `${banner} ${row.inbound_number ?? row.inbound_order_id} ${row.sku_code} ${row.batch_lot ?? ''}`.trim()
      )
      console.log(
      ...
      SET
        ...
        status = CASE
          WHEN (
            ...
            THEN 'POSTED'::"InboundOrderLineStatus"
          ELSE 'PENDING'::"InboundOrderLineStatus"
        END,
        updated_at = CURRENT_TIMESTAMP
      FROM inbound_orders po
      WHERE inbound.id = pol.inbound_order_id
  ...
  console.log(`${banner} updated Inbound lines=${linesUpdate.rowCount}`)
  ...
  const txUpdate = await client.query(
    `
      ...
      FROM inbound_order_lines pol
      JOIN inbound_orders po ON inbound.id = pol.inbound_order_id
      WHERE it.inbound_order_line_id = pol.id
