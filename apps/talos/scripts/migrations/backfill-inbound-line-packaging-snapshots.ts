...
function showHelp() {
  console.log(`
Backfill Inbound line packaging snapshots

Copies carton dims/weight + per-pallet config from sku_batches (fallback to skus for dims/weight)
into inbound_order_lines snapshot columns so Inbounds stop depending on live SKU/batch enrichment.

Also backfills inbound_orders.counterparty_address from suppliers where missing.

Usage:
  pnpm --filter @targon/talos tsx scripts/migrations/backfill-inbound-line-packaging-snapshots.ts [options]
...
async function applyForSchema(
  ...
  const hasInboundLines = await tableExists(client, schema, 'inbound_order_lines')
  if (!hasInboundLines) {
    console.log(`${banner} missing inbound_order_lines table; skipping`)
    await client.query('ROLLBACK')
    return
  }

  const hasSnapshotColumns = await columnExists(client, schema, 'inbound_order_lines', 'carton_weight_kg')
  ...
  if (hasSuppliers && hasCounterpartyAddress) {
    const missingInboundAddress = await client.query<{ count: string }>(
      `
        SELECT COUNT(*)::text AS count
        FROM inbound_orders po
        WHERE inbound.counterparty_name IS NOT NULL
          AND (inbound.counterparty_address IS NULL OR length(trim(inbound.counterparty_address)) = 0)
      `
    )
    const missingCount = Number.parseInt(missingInboundAddress.rows[0]?.count ?? '0', 10)
    console.log(`${banner} inbound_orders missing counterparty_address=${missingCount}`)
    ...
    if (!options.dryRun && missingCount > 0) {
      await client.query(
        `
          ...
          FROM suppliers s
          WHERE inbound.counterparty_name = s.name
            AND inbound.counterparty_name IS NOT NULL
            AND (inbound.counterparty_address IS NULL OR length(trim(inbound.counterparty_address)) = 0)
  ...
  console.log(`${banner} inbound_order_lines needing snapshot backfill=${missingCount}`)
