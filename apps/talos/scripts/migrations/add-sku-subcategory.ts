...
Adds skus.subcategory.

Usage:
  pnpm --filter @targon/talos tsx scripts/migrations/add-sku-subcategory.ts [options]
...
async function applyForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  const statement = `ALTER TABLE "skus" ADD COLUMN IF NOT EXISTS "subcategory" text`
  console.log(`\n[${tenant}] Ensuring skus.subcategory exists`)
  if (options.dryRun) {
