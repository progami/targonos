...
/**
 * Audit PO documents after the 2026-02 "reshuffle" that moved certain document types one stage earlier.
 *
 ...
 *
 * Usage:
 *   pnpm --filter @targon/talos exec tsx scripts/audits/audit-po-document-reshuffle.ts [options]
 *
 * Options:
...
function showHelp() {
  console.log('=== PO Document Reshuffle Audit ===\n')
  console.log('Options:')
...
async function runForTenant(tenant: TenantCode, options: ScriptOptions) {
  ...
  for (const finding of FINDINGS) {
    ...
    const count = await prisma.purchaseOrderDocument.count({
      where: {
    ...
    const sample = await prisma.purchaseOrderDocument.findMany({
      where: {
        stage: finding.where.stage as never,
        documentType: docTypeFilter as never,
      },
    ...
    for (const row of sample) {
      console.log(
        `[${tenant}] ${row.purchaseOrderId} | ${row.stage} | ${row.documentType} | ${row.fileName} | ${row.uploadedAt.toISOString()} | ${row.uploadedByName ?? '—'}`
      )
    }
  }
