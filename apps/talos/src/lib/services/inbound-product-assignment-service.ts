...
export type InboundProductAssignmentRecord = {
  userEmail: string
  skuCode: string
  createdAt: Date
...
export async function listAssignmentsByEmail(email: string): Promise<InboundProductAssignmentRecord[]> {
  const normalizedEmail = normalizeEmail(email)
  const prisma = await getTenantPrisma()

  return prisma.$queryRaw<InboundProductAssignmentRecord[]>(Prisma.sql`
    SELECT
      user_email AS "userEmail",
      sku_code AS "skuCode",
      created_at AS "createdAt",
      created_by_email AS "createdByEmail"
    FROM "inbound_product_assignments"
...
export async function assignSkuToEmail(
  ...
  skuCode: string,
  createdByEmail: string
): Promise<InboundProductAssignmentRecord> {
  const normalizedEmail = normalizeEmail(email)
  const prisma = await getTenantPrisma()

  const rows = await prisma.$queryRaw<InboundProductAssignmentRecord[]>(Prisma.sql`
    INSERT INTO "inbound_product_assignments" (
      user_email,
      sku_code,
      created_by_email
    )
  ...
  if (!assignment) {
    throw new Error('Failed to upsert Inbound product assignment')
  }
...
export async function unassignSkuFromEmail(email: string, skuCode: string): Promise<void> {
  ...
  await prisma.$executeRaw(Prisma.sql`
    DELETE FROM "inbound_product_assignments"
    WHERE user_email = ${normalizedEmail}
