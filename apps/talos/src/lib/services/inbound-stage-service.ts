...
import {
  ...
  InboundOrder,
  InboundOrderProformaInvoice,
  InboundOrderLine,
  InboundOrderStatus,
  InboundOrderLineStatus,
  InboundOrderDocumentStage,
  TransactionType,
  InboundReceiveType,
  Grn,
  Prisma,
...
import {
  ...
  buildLotReference,
  buildInboundOrderReference,
  getNextCommercialInvoiceSequence,
  getNextInboundOrderSequence,
  isInboundOrderReferenceUsedAcrossTenants,
  normalizeSkuGroup,
  parseOrderReference,
...
import { resolveInboundOrderDestination } from '@/lib/inbound/destination'
...
type InboundOrderWithLines = InboundOrder & {
  lines: InboundOrderLine[]
  proformaInvoices?: InboundOrderProformaInvoice[]
}
type InboundOrderGrnReference = Pick<Grn, 'referenceNumber' | 'receivedAt' | 'createdAt'>
type InboundOrderWithOptionalLines = InboundOrder & {
  lines?: InboundOrderLine[]
  proformaInvoices?: InboundOrderProformaInvoice[]
  grns?: InboundOrderGrnReference[]
}

type ManufacturingStageData = {
  proformaInvoiceNumber: InboundOrder['proformaInvoiceNumber']
  proformaInvoiceDate: InboundOrder['proformaInvoiceDate']
  factoryName: InboundOrder['factoryName']
  manufacturingStartDate: InboundOrder['manufacturingStartDate']
  expectedCompletionDate: InboundOrder['expectedCompletionDate']
  actualCompletionDate: InboundOrder['actualCompletionDate']
  totalWeightKg: number | null
  totalVolumeCbm: number | null
  totalCartons: InboundOrder['totalCartons']
  totalPallets: InboundOrder['totalPallets']
  packagingNotes: InboundOrder['packagingNotes']
  proformaInvoiceId: InboundOrder['proformaInvoiceId']
  proformaInvoiceData: InboundOrder['proformaInvoiceData']
  manufacturingStart: InboundOrder['manufacturingStart']
  manufacturingEnd: InboundOrder['manufacturingEnd']
  cargoDetails: InboundOrder['cargoDetails']
}

type OceanStageData = {
  houseBillOfLading: InboundOrder['houseBillOfLading']
  masterBillOfLading: InboundOrder['masterBillOfLading']
  commercialInvoiceNumber: InboundOrder['commercialInvoiceNumber']
  packingListRef: InboundOrder['packingListRef']
  vesselName: InboundOrder['vesselName']
  voyageNumber: InboundOrder['voyageNumber']
  portOfLoading: InboundOrder['portOfLoading']
  portOfDischarge: InboundOrder['portOfDischarge']
  estimatedDeparture: InboundOrder['estimatedDeparture']
  estimatedArrival: InboundOrder['estimatedArrival']
  actualDeparture: InboundOrder['actualDeparture']
  actualArrival: InboundOrder['actualArrival']
  commercialInvoiceId: InboundOrder['commercialInvoiceId']
}

type WarehouseStageData = {
  warehouseCode: InboundOrder['warehouseCode']
  warehouseName: InboundOrder['warehouseName']
  customsEntryNumber: InboundOrder['customsEntryNumber']
  customsClearedDate: InboundOrder['customsClearedDate']
  dutyAmount: number | null
  dutyCurrency: InboundOrder['dutyCurrency']
  surrenderBlDate: InboundOrder['surrenderBlDate']
  transactionCertNumber: InboundOrder['transactionCertNumber']
  receivedDate: InboundOrder['receivedDate']
  discrepancyNotes: InboundOrder['discrepancyNotes']
  warehouseInvoiceId: InboundOrder['warehouseInvoiceId']
  surrenderBL: InboundOrder['surrenderBL']
  transactionCertificate: InboundOrder['transactionCertificate']
  customsDeclaration: InboundOrder['customsDeclaration']
}
...
// Valid stage transitions for current Inbound workflow (RFQ stage removed; legacy RFQ rows are treated as ISSUED)
export const VALID_TRANSITIONS: Partial<Record<InboundOrderStatus, InboundOrderStatus[]>> = {
  ISSUED: [InboundOrderStatus.MANUFACTURING, InboundOrderStatus.CANCELLED],
  MANUFACTURING: [InboundOrderStatus.OCEAN, InboundOrderStatus.CANCELLED],
  OCEAN: [InboundOrderStatus.WAREHOUSE, InboundOrderStatus.CANCELLED],
  WAREHOUSE: [InboundOrderStatus.CANCELLED],
...
export const STAGE_REQUIREMENTS: Record<string, string[]> = {
  // Issued = Inbound issued to supplier
...
export const STAGE_DOCUMENT_REQUIREMENTS: Partial<Record<InboundOrderStatus, string[]>> = {
...
const STAGE_EDITABLE_FIELDS: Partial<
  Record<InboundOrderStatus, Array<keyof StageTransitionInput>>
> = {
  [InboundOrderStatus.ISSUED]: [
  ...
  [InboundOrderStatus.MANUFACTURING]: [
...
function filterStageDataForTarget(
  targetStatus: InboundOrderStatus,
...
function applyStageFieldDataToOrderUpdate(
  updateData: Prisma.InboundOrderUpdateInput,
  stageData: StageTransitionInput
): void {
...
export function isValidTransition(
  fromStatus: InboundOrderStatus,
  toStatus: InboundOrderStatus
): boolean {
  const normalizedStatus = normalizeWorkflowStatus(fromStatus)
  if (normalizedStatus === InboundOrderStatus.CANCELLED) {
    return false
  }

  if (!isCancelableInboundOrderStatus(normalizedStatus)) {
    return false
  }

  const validTargets = getValidNextInboundOrderStatuses(normalizedStatus)
 ...
 */
export function getValidNextStages(currentStatus: InboundOrderStatus): InboundOrderStatus[] {
  ...
  return getValidNextInboundOrderStatuses(normalizedStatus) as InboundOrderStatus[]
}

/**
 * Get required fields for transitioning to a stage
 */
export function getRequiredFieldsForStage(stage: InboundOrderStatus): string[] {
...
export function validateStageData(
  targetStage: InboundOrderStatus,
  data: StageTransitionInput,
  existingOrder: InboundOrder
  ...
  for (const field of requiredFields) {
    ...
    const existingValue = existingOrder[field as keyof InboundOrder]
...
async function requireDocuments(params: {
  ...
  const rows = await params.prisma.inboundOrderDocument.findMany({
    where: {
...
async function validateTransitionGate(params: {
  ...
  if (params.targetStatus === InboundOrderStatus.ISSUED) {
    ...
    if (params.order.counterpartyName && params.order.counterpartyName.trim().length > 0) {
      ...
      if (!banking || banking.trim().length === 0) {
        recordGateIssue(
          ...
          'Supplier banking information is required to issue a Inbound'
        )
      }
    }
  ...
  if (params.targetStatus === InboundOrderStatus.MANUFACTURING) {
    const tenant = await getCurrentTenant()
    const tenantCode = tenant.code
    ...
    await requireLinePiDocuments({
      prisma: params.prisma,
      inboundOrderId: params.order.id,
      activeLines,
      issues,
    })
  ...
  if (params.targetStatus === InboundOrderStatus.OCEAN) {
    await requireLinePiDocuments({
      prisma: params.prisma,
      inboundOrderId: params.order.id,
      activeLines,
    ...
    await requireDocuments({
      prisma: params.prisma,
      inboundOrderId: params.order.id,
      stage: InboundOrderDocumentStage.MANUFACTURING,
  ...
  if (params.targetStatus === InboundOrderStatus.WAREHOUSE) {
    const resolveOrderString = (
      key: keyof StageTransitionInput & keyof InboundOrder
    ): string | null => {
    ...
    const oceanFields: Array<{ key: keyof InboundOrder; issueKey: string; label: string }> = [
      {
    ...
    await requireDocuments({
      prisma: params.prisma,
      inboundOrderId: params.order.id,
      stage: InboundOrderDocumentStage.OCEAN,
    ...
    const hasForwardingCost = await params.prisma.inboundOrderForwardingCost.findFirst({
      where: { inboundOrderId: params.order.id },
      select: { id: true },
    })
...
function validateStageDateOrdering(
  ...
  if (targetStatus === InboundOrderStatus.MANUFACTURING) {
  ...
  if (targetStatus === InboundOrderStatus.OCEAN) {
  ...
  if (targetStatus === InboundOrderStatus.WAREHOUSE) {
    if (inboundBaseline) {
      assertNotEarlierThan(
...
async function computeManufacturingCargoTotals(lines: InboundOrderLine[]): Promise<{
  totalCartons: number
  totalPallets: number | null
  totalWeightKg: number | null
  totalVolumeCbm: number | null
}> {
  const activeLines = lines.filter(line => line.status !== InboundOrderLineStatus.CANCELLED)
...
/**
 * Generate the next Inbound reference in sequence.
 * Format: Inbound-<number>-<SKU_GROUP>
 */
export async function generateOrderNumber(skuGroup: string): Promise<string> {
  const prisma = await getTenantPrisma()
  const nextSequence = await getNextInboundOrderSequence(prisma, skuGroup)
  return buildInboundOrderReference(nextSequence, skuGroup)
}

export interface CreateInboundOrderLineInput {
  skuCode: string
...
/**
 * Create a new Inbound in ISSUED status
...
export async function createInboundOrder(
  input: {
    ...
    notes?: string
    lines?: CreateInboundOrderLineInput[]
  },
  user: UserContext
): Promise<InboundOrderWithLines> {
  const prisma = await getTenantPrisma()
  ...
  if (input.lines && input.lines.length > 0) {
    ...
    for (const line of normalizedLines) {
      ...
      if (normalizeInboundCostCurrency(line.currency) !== INBOUND_BASE_CURRENCY) {
        throw new ValidationError(
          `Inbound currency must be ${INBOUND_BASE_CURRENCY} for SKU ${line.skuCode}`
        )
      }
    ...
    if (skuGroups.size !== 1) {
      throw new ValidationError('All SKUs in a inbound must share one SKU group')
    }
  ...
  let order: InboundOrderWithLines | null = null
  ...
  for (let attempt = 0; attempt < MAX_ORDER_NUMBER_ATTEMPTS; attempt += 1) {
    ...
    if (!generatedOrderReference) {
      throw new ValidationError('Unable to generate a valid Inbound reference')
    }
    const referenceAlreadyUsed = await isInboundOrderReferenceUsedAcrossTenants(orderNumber)
                      ...
                      return {
                        ...
                        totalCost:
                          typeof line.totalCost === 'number' && Number.isFinite(line.totalCost)
                            ? normalizeInboundOrderTotalCost(line.totalCost).toFixed(
                                INBOUND_TOTAL_COST_DECIMALS
                              )
                            : undefined,
                        unitCost:
                          ...
                          line.unitsOrdered > 0
                            ? deriveInboundOrderUnitCost(
                                normalizeInboundOrderTotalCost(line.totalCost),
                                line.unitsOrdered
                              )?.toFixed(INBOUND_UNIT_COST_DECIMALS)
                            : undefined,
 ...
 */
export async function transitionInboundOrderStage(
  orderId: string,
  targetStatus: InboundOrderStatus,
  stageData: StageTransitionInput,
  user: UserContext
  ...
  if (!order) {
    throw new NotFoundError(`Inbound not found: ${orderId}`)
  }
  ...
  const rawStatus = order.status as InboundOrderStatus
  ...
  if (isInPlaceUpdate) {
    const canEdit = await hasPermission(user.id, 'inbound.edit')
    if (!canEdit && !isSuperAdmin(user.email)) {
      throw new ValidationError(`You don't have permission to edit inbound`)
    }
  } else if (targetStatus === InboundOrderStatus.CANCELLED) {
    const canCancel = await hasPermission(user.id, 'inbound.cancel')
    if (!canCancel && !isSuperAdmin(user.email)) {
      throw new ValidationError(`You don't have permission to cancel inbound`)
    }
  } else {
  ...
  if (!isInPlaceUpdate && targetStatus === InboundOrderStatus.CANCELLED) {
  ...
  const derivedManufacturingTotals =
    !isInPlaceUpdate && targetStatus === InboundOrderStatus.MANUFACTURING
      ? await computeManufacturingCargoTotals(order.lines)
      : null

  if (targetStatus === InboundOrderStatus.MANUFACTURING && derivedManufacturingTotals) {
  ...
  const updateData: Prisma.InboundOrderUpdateInput = isInPlaceUpdate
    ? {}
    : {
        status: targetStatus,
      }

  if (!isInPlaceUpdate && currentStatus === InboundOrderStatus.ISSUED && !order.inboundNumber) {
    const orderReferenceSeed = resolveOrderReferenceSeed({
      orderNumber: order.orderNumber,
      inboundNumber: order.inboundNumber,
      skuGroup: order.skuGroup,
    })
    updateData.inboundNumber = buildInboundOrderReference(
      orderReferenceSeed.sequence,
      orderReferenceSeed.skuGroup
    )
  }

  if (!isInPlaceUpdate && currentStatus === InboundOrderStatus.ISSUED && !order.rfqApprovedAt) {
  ...
  if (!isInPlaceUpdate && targetStatus === InboundOrderStatus.ISSUED && !order.inboundNumber) {
    const orderReferenceSeed = resolveOrderReferenceSeed({
      orderNumber: order.orderNumber,
      inboundNumber: order.inboundNumber,
      skuGroup: order.skuGroup,
  ...
  if (!isInPlaceUpdate) {
    ...
    const shouldGenerateCommercialInvoiceNumber =
      targetStatus === InboundOrderStatus.OCEAN && existingCommercialInvoiceNumber.length === 0
  ...
  if (!isInPlaceUpdate) {
    switch (targetStatus) {
      ...
      case InboundOrderStatus.OCEAN:
        ...
        break
      case InboundOrderStatus.WAREHOUSE:
  ...
  const isDispatchTransition =
    !isInPlaceUpdate &&
    currentStatus === InboundOrderStatus.MANUFACTURING &&
    targetStatus === InboundOrderStatus.OCEAN
  ...
  type RemainderLineSeed = Omit<Prisma.InboundOrderLineCreateManyInput, 'inboundOrderId'>
  type DispatchSplitPlan = {
    groupId: string
    shippingLineUpdates: Array<{ lineId: string; data: Prisma.InboundOrderLineUpdateInput }>
  ...
  const buildDispatchSplitPlan = async (): Promise<DispatchSplitPlan | null> => {
    ...
    const shippingLineUpdates: Array<{
      lineId: string
      data: Prisma.InboundOrderLineUpdateInput
    }> = []
    const remainderLineSeeds: RemainderLineSeed[] = []

    let hasRemainder = false

    const shippingLinesForTotals: InboundOrderLine[] = []
    const remainderLinesForTotals: InboundOrderLine[] = []
    ...
    for (const line of activeLines) {
      ...
      const resolvedUnitCost = resolveInboundOrderUnitCost({
      ...
      if (remainderCartons > 0) {
        ...
        remainderLineSeeds.push({
          ...
          unitCost: new Prisma.Decimal(unitCost.toFixed(INBOUND_UNIT_COST_DECIMALS)),
          totalCost: new Prisma.Decimal(remainderCostRounded.toFixed(2)),
          currency: INBOUND_BASE_CURRENCY,
          status: InboundOrderLineStatus.PENDING,
          postedQuantity: 0,
  ...
  let updatedOrder: InboundOrder | null = null
      ...
      updatedOrder = await prisma.$transaction(async tx => {
        if (dispatchSplitPlan && remainderOrderNumber) {
          if (!remainderOrderReference) {
            throw new ValidationError('Unable to generate a valid split Inbound reference')
          }
          ...
          const remainder = await tx.inboundOrder.create({
            data: {
              orderNumber: remainderOrderNumber,
              skuGroup: remainderOrderReference.skuGroup,
              inboundNumber: toPublicOrderNumber(remainderOrderNumber),
          ...
          if (dispatchSplitPlan.remainderLineSeeds.length > 0) {
            await tx.inboundOrderLine.createMany({
              data: dispatchSplitPlan.remainderLineSeeds.map(seed => ({
                ...
                inboundOrderId: remainder.id,
              })),
            })
          }

          const documentsToCopy = await tx.inboundOrderDocument.findMany({
            where: {
              inboundOrderId: order.id,
              stage: {
                in: [
                  InboundOrderDocumentStage.RFQ,
                  InboundOrderDocumentStage.ISSUED,
                  InboundOrderDocumentStage.MANUFACTURING,
                ],
              },
            },
          })
          ...
          if (documentsToCopy.length > 0) {
            await tx.inboundOrderDocument.createMany({
              data: documentsToCopy.map(doc => ({
                inboundOrderId: remainder.id,
                stage:
                  doc.stage === InboundOrderDocumentStage.RFQ
                    ? InboundOrderDocumentStage.ISSUED
                    : doc.stage,
          ...
          const invoicesToCopy = await tx.inboundOrderProformaInvoice.findMany({
            where: { inboundOrderId: order.id },
          })

          if (invoicesToCopy.length > 0) {
            await tx.inboundOrderProformaInvoice.createMany({
        ...
        const nextOrder = await tx.inboundOrder.update({
          where: { id: orderId },
          data: updateData,
          include: { lines: true },
        })
        ...
        if (filteredStageData.proformaInvoiceNumber !== undefined) {
          ...
          if (piNumber) {
            ...
            await tx.inboundOrderProformaInvoice.upsert({
              where: {
        ...
        const refreshed = await tx.inboundOrder.findUnique({
          where: { id: nextOrder.id },
          include: { lines: true, proformaInvoices: { orderBy: [{ createdAt: 'asc' }] } },
        })

        if (!refreshed) {
          throw new NotFoundError(`Inbound not found after transition: ${nextOrder.id}`)
        }
  ...
  if (!updatedOrder) {
    throw new ValidationError('Unable to split inbound. Please retry.')
  }
  ...
  if (createdRemainderOrderId) {
    await auditLog({
      ...
      entityType: 'InboundOrder',
      entityId: orderId,
      oldValue: { inboundOrderId: orderId },
      newValue: {
        splitGroupId: dispatchSplitPlan?.groupId ?? null,
        remainderInboundOrderId: createdRemainderOrderId,
      },
    })
  }
...
export interface ReceiveInboundOrderInventoryInput {
  warehouseCode: string
...
export async function receiveInboundOrderInventory(params: {
  orderId: string
  input: ReceiveInboundOrderInventoryInput
  user: UserContext
}): Promise<InboundOrder> {
  const prisma = await getTenantPrisma()
  ...
  if (!order) {
    throw new NotFoundError(`Inbound not found: ${params.orderId}`)
  }
  ...
  if (order.postedAt) {
    throw new ConflictError('Inventory has already been received for this inbound')
  }

  const tenantCostCurrency = INBOUND_BASE_CURRENCY
  ...
  const supplierDiscrepancyAdjustment = (() => {
    ...
    const sourceId = `inbound_receiving_discrepancy:${order.id}`
  ...
  if (warehouse) {
    const forwardingCost = await prisma.inboundOrderForwardingCost.findFirst({
      where: { inboundOrderId: order.id },
      select: { id: true },
    })
    if (!forwardingCost) {
  ...
  const updatedOrder = await prisma.$transaction(async tx => {
    ...
    if (existingCount > 0) {
      throw new ConflictError('Inventory transactions already exist for this inbound')
    }
      ...
      if (cartons === 0) {
        await tx.inboundOrderLine.update({
          ...
          data: {
            postedQuantity: 0,
            quantityReceived: 0,
            status: InboundOrderLineStatus.POSTED,
          },
        })
        continue
      }
    ...
    if (forwardingCosts.length > 0) {
      ...
      try {
        forwardingLedgerEntries = forwardingCosts.flatMap(cost => {
          const resolvedCurrency = normalizeInboundCostCurrency(cost.currency) ?? tenantCostCurrency
          return buildInboundForwardingCostLedgerEntries({
    ...
    const dutyAmount =
      typeof params.input.dutyAmount === 'number' && Number.isFinite(params.input.dutyAmount)
        ...
        : null
    const dutyCurrency = dutyAmount ? INBOUND_BASE_CURRENCY : null
    ...
    if (!refreshed) {
      throw new NotFoundError(`Inbound not found after receiving: ${order.id}`)
    }
...
export async function generateInboundOrderShippingMarks(params: {
  orderId: string
  user: UserContext
}): Promise<string> {
  const prisma = await getTenantPrisma()
  ...
  const labels = activeLines.map(line => {
    ...
    return `
      <div class="label">
        ...
        <div class="label-row"><span class="k">Inbound</span><span class="v">${inboundNumber}</span></div>
...
/**
 * Get stage approval history for a Inbound
 */
export function getStageApprovalHistory(order: InboundOrder): {
  stage: string
 ...
 */
export function getStageData(order: InboundOrder): StageData {
  return {
    manufacturing: {
      proformaInvoiceNumber: order.proformaInvoiceNumber,
      proformaInvoiceDate: order.proformaInvoiceDate,
...
function getLatestGrnNumber(order: InboundOrderWithOptionalLines): string | null {
...
export function serializeInboundOrder(
  ...
  return {
    ...
    type: order.type,
    status: normalizeWorkflowStatus(order.status as InboundOrderStatus),
    warehouseCode: order.warehouseCode,
    warehouseName: order.warehouseName,
    counterpartyName: order.counterpartyName,
    ...
    outputs: {
      ...
      inboundPdf: {
        generatedAt: inboundPdfGeneratedAt ? inboundPdfGeneratedAt.toISOString() : null,
        generatedByName: order.inboundPdfGeneratedByName ?? null,
        outOfDate: inboundPdfGeneratedAt ? inboundPdfGeneratedAt < lastChangedAt : false,
      },
      shippingMarks: {
