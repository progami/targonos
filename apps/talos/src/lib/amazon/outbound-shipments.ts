export const AMAZON_OUTBOUND_SHIPMENT_STATUSES = [
  'WORKING',
  'READY_TO_SHIP',
  'SHIPPED',
  'IN_TRANSIT',
...
export type AmazonOutboundShipmentStatus = (typeof AMAZON_OUTBOUND_SHIPMENT_STATUSES)[number]

export type AmazonOutboundShipmentsQuery =
  | {
      QueryType: 'SHIPMENT'
      MarketplaceId: string
      ShipmentStatusList: readonly AmazonOutboundShipmentStatus[]
    }
  | {
...
export function resolveOutboundShipmentsMarketplaceId(
...
export function buildOutboundShipmentsQuery(
  marketplaceId: string,
  nextToken?: string
): AmazonOutboundShipmentsQuery {
...
type OutboundShipmentPage = {
  shipments: unknown[]
  nextToken: string | null
}

export type AmazonOutboundShipmentListRow = {
  shipmentId: string
  shipmentName: string
  shipmentStatus: AmazonOutboundShipmentStatus
...
function getRequiredShipmentStatus(record: Record<string, unknown>): AmazonOutboundShipmentStatus {
  const status = getRequiredStringField(record, 'ShipmentStatus')
  const knownStatus = AMAZON_OUTBOUND_SHIPMENT_STATUSES.find(value => value === status)
  if (!knownStatus) {
...
export function normalizeOutboundShipmentListRow(value: unknown): AmazonOutboundShipmentListRow {
...
export function extractOutboundShipmentsResponsePage(response: unknown): OutboundShipmentPage {
...
export async function collectOutboundShipmentPages(
  ...
  while (true) {
    ...
    const page = extractOutboundShipmentsResponsePage(response)
