...
export const GET = withAuth(async (request, session) => {
  ...
  const currentInventory =
    (inventoryStats._sum.cartonsIn || 0) - (inventoryStats._sum.cartonsOut || 0)
  const lastMonthInventory =
    (transactionsUpToLastMonth._sum.cartonsIn || 0) -
  ...
  const costChange = lastCost > 0 ? ((currentCost - lastCost) / lastCost) * 100 : 0

  const activeSkusCount = activeSkusGroup.filter(
    sku => (sku._sum.cartonsIn || 0) - (sku._sum.cartonsOut || 0) > 0
  ).length
  ...
  if (_timeRange === 'allTime') {
    const firstInventoryTransaction = await prisma.inventoryTransaction.findFirst({
      where: warehouseFilter,
      select: { transactionDate: true },
      orderBy: { transactionDate: 'asc' },
    })
    trendStartDate = firstInventoryTransaction?.transactionDate
      ? new Date(firstInventoryTransaction.transactionDate)
      : new Date()
  ...
  runningBalance =
    (initialBalanceData._sum.cartonsIn || 0) - (initialBalanceData._sum.cartonsOut || 0)
  ...
  const totalCartons = warehouseInventoryGroups.reduce(
    (sum, w) => sum + ((w._sum.cartonsIn || 0) - (w._sum.cartonsOut || 0)),
    0
  )
  ...
  for (const alert of skusWithFbaData) {
    ...
    if (alert.status === 'MISMATCH') {
      ...
      alert.status === 'MISSING_REFERENCE' ||
      alert.status === 'NO_ASIN' ||
      alert.status === 'ERROR'
    ) {
