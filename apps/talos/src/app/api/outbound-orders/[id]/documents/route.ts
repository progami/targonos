...
import { OutboundOrderDocumentStage, Prisma } from '@targon/prisma-talos'
...
export const POST = withAuthAndParams(async (request, params, session) => {
  ...
  let stage: OutboundOrderDocumentStage | null = null
  ...
  try {
    ...
    const validation = isJson
      ...
      : await validateFile(file as File, 'outbound-order-document', { maxSizeMB: MAX_DOCUMENT_SIZE_MB })
    if (!validation.valid) {
    ...
    const outboundOrderNumber = typeof order.outboundNumber === 'string' && order.outboundNumber.trim() ? order.outboundNumber.trim() : null

    let s3Key: string
    let uploadSize: number

    if (isJson) {
      const folder = outboundOrderNumber ? `${outboundOrderNumber.toLowerCase()}--${id}` : id
      const expectedPrefix = `outbound-orders/${tenantCode.toLowerCase()}/${folder}/${stage}/${documentType}/`
      ...
      const generatedKey = s3Service.generateKey(
        {
          ...
          outboundOrderId: id,
          tenantCode,
          outboundOrderNumber: outboundOrderNumber ?? undefined,
          stage: stage as OutboundOrderDocumentStage,
    ...
    const compositeKey = {
      outboundOrderId_stage_documentType: {
        outboundOrderId: id,
        stage: stage as OutboundOrderDocumentStage,
        documentType: documentType as string,
      },
    ...
    const existing = await prisma.outboundOrderDocument.findUnique({
      where: compositeKey,
      select: { s3Key: true },
    })
    ...
    const stored = await prisma.outboundOrderDocument.upsert({
      where: compositeKey,
      create: {
        outboundOrderId: id,
        stage: stage as OutboundOrderDocumentStage,
    ...
    apiLogger.error('Failed to upload outbound order document', {
      outboundOrderId,
      stage,
      documentType,
      fileName,
    ...
    return NextResponse.json(
      {
        error: 'Failed to upload outbound order document',
...
export const GET = withAuthAndParams(async (request, params, _session) => {
  try {
    ...
    const docs = await prisma.outboundOrderDocument.findMany({
    ...
    return NextResponse.json(
      {
        error: 'Failed to fetch outbound order documents',
        details: _error instanceof Error ? _error.message : 'Unknown error',
      },
      { status: 500 }
    )
