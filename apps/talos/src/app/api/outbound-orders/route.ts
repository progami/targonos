...
const CreateOutboundOrderSchema = z.object({
  ...
  destinationType: z.nativeEnum(OutboundDestinationType).optional(),
 ...
 * Create a new Outbound Order (Outbound) in DRAFT status.
 */
export const POST = withAuth(async (request: NextRequest, session) => {
  const payload = await request.json().catch(() => null)
  const result = CreateOutboundOrderSchema.safeParse(payload)
  ...
  const canCreate = await hasPermission(userContext.id, 'outbound.create')
  if (!canCreate) {
  ...
  try {
    const order = await createOutboundOrder(input, userContext)
