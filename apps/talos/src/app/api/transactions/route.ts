  ...
  try {
    ...
    if (hasInlineSkuCreation) {
      return NextResponse.json(
        {
          error:
            'Creating new SKUs from transactions is disabled. Import SKUs from Amazon → SKU Info first.',
        },
        { status: 400 }
      )
    }
    ...
    if (
      ...
      bodyWarehouseId &&
      bodyWarehouseId !== session.user.warehouseId
    ) {
      return NextResponse.json(
        { error: 'Staff users cannot specify a different warehouse' },
        { status: 403 }
      )
    }
 ...
 */
    ...
    const maxPalletHeightMm =
      typeof whBillingConfig?.maxPalletHeightMm === 'number'
        ? whBillingConfig.maxPalletHeightMm
        : undefined
    ...
    for (const item of validatedItems) {
      ...
      if (!sku.isActive) {
        return NextResponse.json(
          {
            error: `SKU ${sku.skuCode} is inactive. Reactivate it in Amazon → SKU Info first.`,
          },
          { status: 400 }
        )
      }
    ...
    const result = await prisma.$transaction(async tx => {
        ...
        const transaction = await tx.inventoryTransaction.create({
          data: {
            ...
            attachments: (() => {
              ...
              const combinedAttachments: Record<string, unknown> =
      ...
      if (txType === 'RECEIVE' || txType === 'SHIP') {
        ...
        if (costLedgerEntries.length > 0) {
          ...
          const financialEntries: Prisma.FinancialLedgerEntryCreateManyInput[] = inserted.map(
            row => {
