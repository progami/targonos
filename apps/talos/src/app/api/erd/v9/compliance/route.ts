...
import { ERD_V9_ENTITIES, ERD_V9_ENTITY_COLUMNS, type ErdV9Entity } from '@/lib/erd/v9'
...
export const GET = withAuth(async () => {
  ...
  const entityValues = ERD_V9_ENTITIES.map(entity => Prisma.sql`${entity}`)
  ...
  const entities: EntityCompliance[] = ERD_V9_ENTITIES.map(entity => {
    const expected = ERD_V9_ENTITY_COLUMNS[entity]
