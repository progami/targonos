...
/**
 * GET /api/inbound/[id]/lines/[lineId]
...
export const GET = withAuthAndParams(async (request: NextRequest, params, _session) => {
  ...
  const line = await prisma.inboundOrderLine.findFirst({
    where: {
      id: lineId,
      inboundOrderId: id,
    },
...
/**
 * PATCH /api/inbound/[id]/lines/[lineId]
 * Update a line item
 */
export const PATCH = withAuthAndParams(async (request: NextRequest, params, _session) => {
  ...
  const updateData: Prisma.InboundOrderLineUpdateInput = {}
  try {
  ...
  if (allowCommercialEdits) {
    const commercialUpdateData: Prisma.InboundOrderLineUpdateInput = {}
    ...
    const normalizedTotalCost =
      typeof nextTotalCost === 'number' && Number.isFinite(nextTotalCost)
        ? normalizeInboundOrderTotalCost(nextTotalCost)
        : null
    ...
    if ((totalCostChanged || unitsChanged) && normalizedTotalCost !== null) {
      ...
      if (nextUnitsOrdered > 0) {
        commercialUpdateData.unitCost = deriveInboundOrderUnitCost(
          normalizedTotalCost,
          nextUnitsOrdered
    ...
    if (Object.keys(commercialUpdateData).length > 0) {
      commercialUpdateData.currency = INBOUND_BASE_CURRENCY
  ...
  let updated
  try {
    updated = await prisma.inboundOrderLine.update({
      where: { id: lineId },
      data: updateData,
    })
  } catch (error) {
  ...
  const after = {
    ...
    totalCost: toInboundOrderTotalCostNumberOrNull(updated.totalCost),
...
/**
 * DELETE /api/inbound/[id]/lines/[lineId]
 * Delete a line item
 */
export const DELETE = withAuthAndParams(async (request: NextRequest, params, _session) => {
  ...
  await prisma.inboundOrderLine.delete({
    where: { id: lineId },
  })
