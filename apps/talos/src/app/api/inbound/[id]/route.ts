...
import { serializeInboundOrder as serializeWithStageData } from '@/lib/services/inbound-stage-service'
...
export const PATCH = withAuthAndParams(async (request, params, session) => {
  ...
  try {
    const updated = await updateInboundOrderDetails(id, normalized, {
      id: session.user.id,
