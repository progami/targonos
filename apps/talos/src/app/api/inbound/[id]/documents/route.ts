  ...
  try {
    ...
    if (!DISABLE_PO_DOCUMENT_STAGE_LOCK) {
      ...
      if (
        currentStage &&
        stage &&
        INBOUND_DOCUMENT_STAGE_ORDER[stage] <
    ...
    const validation = isJson
      ...
      : await validateFile(file as File, 'inbound-document', { maxSizeMB: MAX_DOCUMENT_SIZE_MB })
    if (!validation.valid) {
    ...
    if (isJson) {
      const expectedPrefix = `inbound/${tenantCode.toLowerCase()}/${inboundOrderNumber.toLowerCase()}--${id}/${stage}/${documentType}/`
    ...
    const existing = await prisma.inboundOrderDocument.findUnique({
      where: compositeKey,
      select: {
        id: true,
        stage: true,
    ...
    const stored = await prisma.inboundOrderDocument.upsert({
      where: compositeKey,
      create: {
        inboundOrderId: id,
        stage,
    ...
    apiLogger.error('Failed to upload inbound document', {
      inboundOrderId,
      stage,
      documentType,
      fileName,
  ...
  try {
    ...
    const withUrls = await Promise.all(
      docs.map(async doc => {
        ...
        return {
          id: doc.id,
          stage: getActiveInboundOrderDocumentStageFromStoredStage(doc.stage),
    ...
    return NextResponse.json(
      {
        error: 'Failed to fetch inbound documents',
        details: _error instanceof Error ? _error.message : 'Unknown error',
      },
      { status: 500 }
    )
