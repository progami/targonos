...
import { getInboundOrderById } from '@/lib/services/inbound-service'
...
<head>
  ...
  <title>Inbound ${escapeHtml(params.documentNumber)}</title>
  <style>
    * {
      margin: 0;
      padding: 0;
  ...
  <div class="page">
    <div class="page-content">
      <div class="header">
        ...
        <div class="inbound-title-section">
          <div class="inbound-title">INBOUND</div>
          <div class="inbound-meta">
            <div class="inbound-meta-row">
              <span class="inbound-meta-label">Inbound Number:</span>
              <span class="inbound-meta-value">${escapeHtml(params.documentNumber)}</span>
            </div>
            <div class="inbound-meta-row">
              <span class="inbound-meta-label">Date:</span>
              <span class="inbound-meta-value">${formatDate(params.createdAt)}</span>
            </div>
            <div class="inbound-meta-row">
...
export const GET = withAuthAndParams(async (_request, params, _session) => {
  ...
  const order = await getInboundOrderById(id)
  if (!order) {
  ...
  // Prefer snapshot supplier address stored on the Inbound. Fall back to live supplier for older Inbound.
  ...
  const destinationWarehouse = order.warehouseCode
    ...
    : null
  const destination = resolveInboundOrderDestination(
    {
