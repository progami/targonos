...
const SOURCE_PREFIX = 'inbound_warehouse_cost'

function buildSourceId(inboundOrderId: string, entryId: string) {
  return `${SOURCE_PREFIX}:${inboundOrderId}:${entryId}`
}

function isOwnedSourceId(sourceId: string, inboundOrderId: string) {
  return sourceId.startsWith(`${SOURCE_PREFIX}:${inboundOrderId}:`)
}
