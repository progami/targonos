...
import { receiveInboundOrderInventory, serializeInboundOrder } from '@/lib/services/inbound-stage-service'
import type { ReceiveInboundOrderInventoryInput, UserContext } from '@/lib/services/inbound-stage-service'
...
const ReceiveInboundOrderSchema = z.object({
...
export const POST = withAuthAndParams(async (request: NextRequest, params, session) => {
  ...
  const parsed = ReceiveInboundOrderSchema.safeParse(payload)
  if (!parsed.success) {
  ...
  try {
    const updated = await receiveInboundOrderInventory({
      orderId: id,
      input: parsed.data as ReceiveInboundOrderInventoryInput,
      user: userContext,
    })
