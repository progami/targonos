...
import {
  getInboundOrders,
  getInboundOrdersBySplitGroup,
} from '@/lib/services/inbound-service'
import {
  createInboundOrder,
  serializeInboundOrder as serializeNewInbound,
...
export const GET = withAuth(async (request: NextRequest, _session) => {
  ...
  const orders =
    typeof splitGroupId === 'string' && splitGroupId.trim().length > 0
      ? await getInboundOrdersBySplitGroup(splitGroupId)
      : await getInboundOrders()
  return ApiResponses.success({
    data: orders.map(order =>
      serializeNewInbound(order, { defaultCurrency: INBOUND_BASE_CURRENCY })
    ),
  })
})
...
/**
 * POST /api/inbound
 * Create a new inbound in ISSUED status with an auto-generated order number.
 * Inbound number is assigned at creation.
 * Warehouse is NOT required at this stage - selected at Stage 4 (WAREHOUSE)
 */
export const POST = withAuth(async (request: NextRequest, session) => {
  const payload = await request.json().catch(() => null)
  const result = CreateInboundSchema.safeParse(payload)
  ...
  const canCreate = await hasPermission(userContext.id, 'inbound.create')
  if (!canCreate) {
