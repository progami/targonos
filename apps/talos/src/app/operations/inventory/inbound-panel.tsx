...
import {
  INBOUND_TYPE_BADGE_CLASSES,
  type InboundType,
} from '@/lib/constants/status-mappings'
import {
  getInboundOrderDisplayStatus,
  type ActiveInboundOrderStatus,
} from '@/lib/inbound/workflow'
import { withBasePath } from '@/lib/utils/base-path'

export type InboundOrderTypeOption = 'PURCHASE' | 'ADJUSTMENT' | 'FULFILLMENT'
export type InboundOrderStatusOption = ActiveInboundOrderStatus
export type InboundOrderLineStatusOption = 'PENDING' | 'POSTED' | 'CANCELLED'

export interface InboundOrderLineSummary {
  id: string
  skuCode: string
  ...
  status: InboundOrderLineStatusOption
...
export interface InboundOrderSummary {
  ...
  stageData: InboundOrderStageData
  createdAt: string
  createdByName: string | null
  lines: InboundOrderLineSummary[]
}

export type InboundOrderFilter = InboundOrderStatusOption

interface InboundOrdersPanelProps {
  onPosted: () => void
  statusFilter?: InboundOrderFilter
  typeFilter?: InboundOrderTypeOption
...
function typeBadgeClasses(type: InboundOrderTypeOption) {
  return INBOUND_TYPE_BADGE_CLASSES[type as InboundType] ?? DEFAULT_BADGE_CLASS
}
...
function sumLineUnits(lines: InboundOrderLineSummary[]) {
  return lines.reduce((sum, line) => sum + (line.unitsOrdered ?? 0), 0)
}

function sumLineCartons(lines: InboundOrderLineSummary[]) {
  return lines.reduce((sum, line) => sum + line.quantity, 0)
}

function sumReceivedQuantities(lines: InboundOrderLineSummary[]) {
  return lines.reduce((sum, line) => sum + (line.quantityReceived ?? line.postedQuantity ?? 0), 0)
}

function getDisplayStatus(status: string): InboundOrderStatusOption {
  return getInboundOrderDisplayStatus(status)
}
...
type TableColumn = {
  ...
  thClassName?: string
  tdClassName?: string
  render: (order: InboundOrderSummary) => ReactNode
}
...
function orderMatchesSearch(order: InboundOrderSummary, term: string) {
...
export function InboundOrdersPanel({
  onPosted: _onPosted,
  statusFilter = 'ISSUED',
  typeFilter,
  onCountsLoaded,
  globalSearch,
  lifecycleTrigger,
}: InboundOrdersPanelProps) {
  const [allOrders, setAllOrders] = useState<InboundOrderSummary[]>([])
  ...
  const [lifecycleMembers, setLifecycleMembers] = useState<InboundOrderSummary[]>([])
  const [lifecycleLoading, setLifecycleLoading] = useState(false)

  const fetchOrders = useCallback(async () => {
    try {
      setLoading(true)
      const response = await fetch(withBasePath('/api/inbound'), { credentials: 'include' })
      if (!response.ok) {
        const payload = await response.json().catch(() => null)
        toast.error(payload?.error ?? 'Failed to load inbound')
        return
      }
      const payload = await response.json().catch(() => null)
      setAllOrders(Array.isArray(payload?.data) ? (payload.data as InboundOrderSummary[]) : [])
    } catch (_error) {
  ...
  const openLifecycle = useCallback(async (order: InboundOrderSummary) => {
    setLifecycleOrderId(order.id)

    if (order.splitGroupId) {
      // Fetch all split group members (may include cross-tenant Inbound)
      setLifecycleLoading(true)
      const localMembers = allOrders.filter(o => o.splitGroupId === order.splitGroupId)

      try {
        const endpoint = withBasePath(`/api/inbound?splitGroupId=${encodeURIComponent(order.splitGroupId)}`)
        const response = await fetch(endpoint, { credentials: 'include' })
        if (response.ok) {
          const payload = await response.json().catch(() => null)
          const remote = Array.isArray(payload?.data) ? (payload.data as InboundOrderSummary[]) : []
  ...
  const columns = useMemo<TableColumn[]>(() => {
    ...
    const getOrderHref = (order: InboundOrderSummary) =>
      order.tenantCode
        ? `/operations/inbound/${order.id}?tenant=${encodeURIComponent(order.tenantCode)}`
        : `/operations/inbound/${order.id}`

    const getInboundReference = (order: InboundOrderSummary) => order.inboundNumber ?? order.orderNumber

    const isTransitStage = statusFilter === 'OCEAN'
    const isWarehouseStage = statusFilter === 'WAREHOUSE'
    const primaryIdentifierLabel = isTransitStage ? 'CI #' : isWarehouseStage ? 'GRN #' : 'Inbound #'

    cols.push({
      key: isTransitStage ? 'primary-ci-number' : isWarehouseStage ? 'primary-grn-number' : 'primary-inbound-number',
      ...
      render: order => {
        ...
        return (
          <Link
            ...
            prefetch={false}
          >
            {isTransitStage || isWarehouseStage ? formatTextOrDash(primaryValue) : getInboundReference(order)}
          </Link>
        )
      },
    })
    ...
    if (isTransitStage) {
      cols.push({
        key: 'inbound-number',
        header: buildColumnHeader('Inbound #'),
        fit: true,
        thClassName: 'w-[126px]',
        tdClassName: 'px-2 py-2 font-medium text-foreground min-w-0',
        render: order => (
    ...
    typeFilter,
      ...
      >
        <table className="w-full text-sm">
          ...
          <tbody>
            {loading ? (
              <DataTableEmpty colSpan={columns.length} message="Loading inbound…" />
            ) : filteredOrders.length === 0 ? (
              <DataTableEmpty colSpan={columns.length} message="No inbound found for this stage." />
            ) : (
      ...
      <Dialog open={lifecycleOrderId !== null} onOpenChange={open => { if (!open) setLifecycleOrderId(null) }}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>Inbound Lifecycle</DialogTitle>
            <DialogDescription>
              {lifecycleOrder
                ? `Journey of ${lifecycleOrder.inboundNumber ?? lifecycleOrder.orderNumber}`
                : 'Loading…'}
            </DialogDescription>
          </DialogHeader>
          {lifecycleLoading ? (
...
const STAGE_ORDER: InboundOrderStatusOption[] = ['ISSUED', 'MANUFACTURING', 'OCEAN', 'WAREHOUSE', 'CANCELLED']
...
function stageIndex(status: InboundOrderStatusOption) {
  const idx = STAGE_ORDER.indexOf(status)
  return idx >= 0 ? idx : 0
}

function LifecycleTree({ order, members }: { order: InboundOrderSummary; members: InboundOrderSummary[] }) {
  const poRef = order.inboundNumber ?? order.orderNumber
  const isSplitGroup = members.length > 1

  // Build stage progression for the parent Inbound
  const displayStatus = getDisplayStatus(order.status)
...
function LifecycleNode({ order }: { order: InboundOrderSummary }) {
