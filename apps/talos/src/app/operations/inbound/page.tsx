...
import { InboundOrdersPanel } from '../inventory/inbound-panel'
import { buildAppCallbackUrl, redirectToPortal } from '@/lib/portal'
import type { ActiveInboundOrderStatus } from '@/lib/inbound/workflow'
import { withBasePath } from '@/lib/utils/base-path'
import type { LucideIcon } from 'lucide-react'

type InboundStageStatus = ActiveInboundOrderStatus

type StatusConfig = {
  value: InboundStageStatus
  label: string
  description: string
  icon: LucideIcon
}
...
const PIPELINE_STAGES: StatusConfig[] = [
  {
    value: 'ISSUED',
    label: 'Issued',
    description: 'Inbound created and issued to supplier',
    icon: Send,
  },
  {
    value: 'MANUFACTURING',
...
const TERMINAL_STATUSES: StatusConfig[] = [
  {
    value: 'CANCELLED',
    label: 'Cancelled',
    description: 'Inbounds cancelled before completion',
    icon: XCircle,
  },
]
...
const PAGE_KEY = '/operations/inbound'
...
function OrdersPageContent() {
  ...
  const statusFromUrl = searchParams.get('status') as InboundStageStatus | null
  const persistedStatus = pageState.activeTab as InboundStageStatus | undefined
  const currentStatus: InboundStageStatus =
  ...
  useEffect(() => {
    ...
    if (!session) {
      redirectToPortal('/login', buildAppCallbackUrl('/operations/inbound'))
      return
    }
      ...
      <PageHeaderSection
        ...
        actions={
          <Button asChild className="gap-2">
            <Link href="/operations/inbound/new">
          ...
          <div className="flex items-center gap-2">
            <div className="relative max-w-sm flex-1">
              ...
              <Input
                ...
                placeholder="Search by Inbound#, CI#, GRN#, supplier…"
