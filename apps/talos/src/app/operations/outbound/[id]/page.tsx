...
type OutboundOrderStatus = 'DRAFT' | 'SHIPPED' | 'CANCELLED'
...
type OutboundOrderDocumentStage = 'PACKING' | 'SHIPPING' | 'DELIVERY'

type OutboundOrderDocumentSummary = {
  id: string
  stage: OutboundOrderDocumentStage
  documentType: string
  fileName: string
  contentType: string
  size: number
...
const STATUS_BADGE_CLASSES: Record<OutboundOrderStatus, string> = {
...
function getDocumentLabel(stage: OutboundOrderDocumentStage, documentType: string) {
...
export default function OutboundOrderDetailPage() {
  ...
  const [activeTab, setActiveTab] = useState<OutboundOrderDetailTab>('details')
  ...
  const [documents, setDocuments] = useState<OutboundOrderDocumentSummary[]>([])
  ...
  const [inlinePreviewDocument, setInlinePreviewDocument] =
    useState<OutboundOrderDocumentSummary | null>(null)
  const [previewDocument, setPreviewDocument] = useState<OutboundOrderDocumentSummary | null>(null)

  useEffect(() => {
    if (status === 'loading') return
    if (!session) {
      redirectToPortal('/login', buildAppCallbackUrl(`/operations/outbound/${params.id}`))
      return
    }
  ...
  const fetchOrder = async () => {
    try {
      setLoading(true)
      const response = await fetch(withBasePath(`/api/outbound-orders/${params.id}`), {
        credentials: 'include',
      })
      const payload = await response.json().catch(() => null)
      if (!response.ok) {
        toast.error(payload?.error ?? 'Failed to load outbound order')
        return
      }

      const data = payload?.data as OutboundOrder | undefined
      if (!data) {
        toast.error('Outbound order not found')
        return
      }
      ...
      toast.error('Failed to load outbound order')
    } finally {
      setLoading(false)
    }
  }
  ...
  useEffect(() => {
    ...
    const loadDocuments = async () => {
      try {
        ...
        const response = await fetch(withBasePath(`/api/outbound-orders/${order.id}/documents`), {
          credentials: 'include',
        })
        if (!response.ok) {
          setDocuments([])
        ...
        setDocuments(Array.isArray(list) ? (list as OutboundOrderDocumentSummary[]) : [])
      } catch {
        setDocuments([])
      } finally {
  ...
  const documentsByKey = useMemo(() => {
    const map = new Map<string, OutboundOrderDocumentSummary>()
  ...
  const missingDocuments = useMemo(() => {
    const missing: Array<{ stage: OutboundOrderDocumentStage; id: string; label: string }> = []
    for (const [stage, docs] of Object.entries(DOCUMENT_REQUIREMENTS) as Array<
      [OutboundOrderDocumentStage, Array<{ id: string; label: string }>]
    >) {
  ...
  const tabIssueCounts = useMemo(() => {
    const issues: Record<OutboundOrderDetailTab, number> = {
      details: 0,
      amazon: 0,
      lines: 0,
      freight: 0,
  ...
  const handleDetailsSave = async () => {
    ...
    try {
      ...
      const response = await fetchWithCSRF(`/api/outbound-orders/${order.id}`, {
        method: 'PATCH',
  ...
  const handleAmazonSync = async () => {
    ...
    try {
      ...
      const response = await fetchWithCSRF(`/api/outbound-orders/${order.id}/amazon-sync`, {
        method: 'POST',
      })
  ...
  const handleShip = async () => {
    ...
    try {
      ...
      const response = await fetchWithCSRF(`/api/outbound-orders/${order.id}/stage`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(payload),
      })
      ...
      const data = await response.json().catch(() => null)
      if (!response.ok) {
        toast.error(data?.error ?? 'Failed to ship outbound order')
        return
      }

      toast.success('Outbound order shipped')
      setOrder(data?.data ?? null)
    } catch (_error) {
      toast.error('Failed to ship outbound order')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      const data = await response.json().catch(() => null)
      if (!response.ok) {
        toast.error(data?.error ?? 'Failed to cancel outbound order')
        return
      }
      ...
      toast.error('Failed to cancel outbound order')
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const handleDocumentUpload = async (
    ...
    try {
      ...
      const completeResponse = await fetchWithCSRF(`/api/outbound-orders/${order.id}/documents`, {
        method: 'POST',
        body: formData,
      })
  ...
  const renderDocumentStage = (stage: OutboundOrderDocumentStage, title: string) => {
    const required = DOCUMENT_REQUIREMENTS[stage]
