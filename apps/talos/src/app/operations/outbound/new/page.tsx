...
type OutboundOrderCreateTab = 'details' | 'amazon' | 'lines' | 'freight'
...
export default function NewOutboundOrderPage() {
  ...
  const [activeTab, setActiveTab] = useState<OutboundOrderCreateTab>('details')
  ...
  const tabIssueCounts = useMemo(() => {
    const issues: Record<OutboundOrderCreateTab, number> = {
      details: 0,
      amazon: 0,
      lines: 0,
      freight: 0,
  ...
  useEffect(() => {
    ...
    if (!session) {
      redirectToPortal('/login', buildAppCallbackUrl('/operations/outbound/new'))
      return
    }
  }, [session, status])
    ...
    const loadInventoryOptions = async () => {
      try {
        ...
        const response = await fetch(
          withBasePath(
            `/api/outbound-orders/inventory-options?warehouseId=${encodeURIComponent(selectedWarehouse.id)}`
          ),
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await fetchWithCSRF('/api/outbound-orders', {
        method: 'POST',
      ...
      if (!response.ok) {
        toast.error(data?.error ?? 'Failed to create outbound order')
        return
      }

      const orderId = data?.data?.id
      if (!orderId) {
        toast.error('Outbound order created but missing ID in response')
        return
      }

      toast.success('Outbound order created')
      router.push(`/operations/outbound/${orderId}`)
    } catch (_error) {
      toast.error('Failed to create outbound order')
    } finally {
      setSubmitting(false)
    }
  }
      ...
      <PageContent>
        <div className="flex flex-col gap-6">
          ...
          <div className="flex justify-end gap-3 pt-2">
            <Button variant="outline" onClick={() => router.push('/operations/outbound')}>
              Cancel
            </Button>
            <Button onClick={handleSubmit} disabled={submitting}>
              {submitting ? 'Creating...' : 'Create Outbound Order'}
            </Button>
          </div>
        </div>
      </PageContent>
