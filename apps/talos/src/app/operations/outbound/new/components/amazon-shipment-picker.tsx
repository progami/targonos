...
export function AmazonShipmentPicker({
  ...
  const [shipments, setShipments] = useState<AmazonOutboundShipment[]>([])
  ...
  const loadShipments = useCallback(async () => {
    ...
    try {
      const response = await fetch(withBasePath('/api/amazon/outbound-shipments'), { credentials: 'include' })
      const payload = await response.json().catch(() => null)
      if (!response.ok) {
        throw new Error(payload?.error ?? 'Failed to fetch outbound shipments')
      }
      const list = Array.isArray(payload?.data?.shipments) ? payload.data.shipments : []
      setShipments(list)
    } catch (error) {
      setShipments([])
      setShipmentsError(error instanceof Error ? error.message : 'Failed to fetch outbound shipments')
    } finally {
  ...
  const handleImport = async (shipmentId: string) => {
    ...
    try {
      const response = await fetch(
        withBasePath(`/api/amazon/outbound-shipments/${encodeURIComponent(shipmentId)}`),
        { credentials: 'include' }
      )
      const payload = await response.json().catch(() => null)
      if (!response.ok) {
