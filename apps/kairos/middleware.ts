...
export async function middleware(request: NextRequest) {
  ...
  const decision = await requireAppEntry({
    ...
    cookieNames: Array.from(new Set([
      ...
      ...getCandidateSessionCookieNames('kairos'),
    ])),
  })

  if (!decision.allowed) {
    console.info('[authz][kairos] denied', {
      path: normalizedPath,
    ...
    if (normalizedPath.startsWith('/api/')) {
      ...
      const error = decision.status === 'unauthenticated' ? 'Authentication required' : 'No access to Kairos'
