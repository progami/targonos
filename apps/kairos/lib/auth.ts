...
function requireEnv(name: string): string {
  ...
  if (!value || value.trim() === '') {
    throw new Error(`${name} must be defined for Kairos auth configuration.`);
  }
  return value;
}
...
function resolveAuthOptions(): NextAuthConfig {
  ...
  if (!sharedSecret) {
    throw new Error('PORTAL_AUTH_SECRET or NEXTAUTH_SECRET must be defined for Kairos auth configuration.');
  }
