...
import { safeEqual, syncSellerboardUsDashboard } from '@/lib/integrations/sellerboard';
...
export const POST = async (request: Request) => {
  ...
  const identifier = getRateLimitIdentifier(request, 'sellerboard-dashboard-sync');
  ...
  try {
    ...
    const result = await syncSellerboardUsDashboard({ reportUrl, strategyId });
    return NextResponse.json({
      ok: true,
      ...result,
    ...
    console.error(`[POST /sellerboard/us-dashboard] sync error (strategyId=${strategyId}):`, error);
