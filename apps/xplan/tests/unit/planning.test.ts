...
describe('planning calendar coverage', () => {
  it('generates default weeks when no sales data exists', () => {
    ...
    expect(weeksUk).toHaveLength(366)

    const calendarUk = buildWeekCalendar(weeksUk, 1)
    const segmentsUk = buildYearSegments(calendarUk)

    expect(segmentsUk.map((segment) => segment.year)).toEqual([2021, 2022, 2023, 2024, 2025, 2026, 2027])

    const first2021WeekDateUk = getCalendarDateForWeek(-209, calendarUk)
    ...
    expect(first2021WeekDateUk?.toISOString()).toBe('2020-12-28T00:00:00.000Z')
    ...
    expect(weeksDefault).toHaveLength(366)
    ...
    expect(segmentsDefault.map((segment) => segment.year)).toEqual([2021, 2022, 2023, 2024, 2025, 2026, 2027])

    const first2021WeekDateDefault = getCalendarDateForWeek(-209, calendarDefault)
    ...
    expect(first2021WeekDateDefault?.toISOString()).toBe('2020-12-28T00:00:00.000Z')
