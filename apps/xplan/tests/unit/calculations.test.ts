...
describe('computePurchaseOrderDerived', () => {
  it('calculates landed cost and payment schedule', () => {
    ...
    expect(differenceInCalendarDays(derivedOrder.productionComplete!, productionStart)).toBe(7);
  });
  ...
  it('honours per-order cost overrides', () => {
    ...
    const expectedTariff =
      (overrideOrder.overrideManufacturingCost ?? product.manufacturingCost) * 0.1;
    expect(overridden.landedUnitCost).toBeCloseTo(5 + 2 + expectedTariff);
    expect(overridden.plannedPoValue).toBeCloseTo(
      overridden.landedUnitCost * overrideOrder.quantity,
    );
  });
  ...
  it('preserves manual payment due dates', () => {
    ...
    const freightPayment = derivedManual.plannedPayments.find(
      (payment) => payment.paymentIndex === 3,
    );
...
describe('computeSalesPlan', () => {
  ...
  it('allocates inbound quantities to the correct batch products', () => {
    ...
    const derivedMulti = computePurchaseOrderDerived(
      multiProductOrder,
      productIndexMulti,
      leadProfile,
      parameters,
    ...
    const multiPlan = computeSalesPlan(multiSalesWeeks, [derivedMulti], {
      productIds: [product.id, productTwo.id],
    });
    const widgetArrival = multiPlan.find(
      (row) => row.productId === product.id && row.weekNumber === 3,
    );
    const gadgetArrival = multiPlan.find(
      (row) => row.productId === productTwo.id && row.weekNumber === 3,
    );
  ...
  it('carries inventory forward when future weeks are only provided by the fallback calendar', () => {
    const placeholder: SalesWeekInput[] = [
      {
        id: 'p52',
        productId: '__planning__',
        weekNumber: 52,
        weekDate: new Date('2025-12-22T00:00:00Z'),
      },
      {
        id: 'p53',
        productId: '__planning__',
        weekNumber: 53,
        weekDate: new Date('2025-12-29T00:00:00Z'),
      },
    ];
...
const profitResult = computeProfitAndLoss(salesPlan, productIndex, parameters, []);
...
const cashResult = computeCashFlow(profitResult.weekly, [derivedOrder], parameters, []);
